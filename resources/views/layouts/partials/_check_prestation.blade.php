...
<div class="table-responsive">
  <table id="{{ $folder ? $folder->code : '' }}" class="table sortable table-hove invoice-table" data-url="{{ url('api/update-invoice') }}" >
    <thead>
      <tr>
        <th>#</th>
        <th>Designation</th>
    ...
    <tbody>
      ...
      @php
          ...
          $prestationable = $invoice->prestationable
      @endphp
...
@endif
      ...
      <tr class="{{ $invoice_line->get_coverage_class() }}" id="{{ $invoice_line->code }}" data-invoice="{{ $invoice->code }}" data-invoice-line="{{ $invoice_line->code }}"
        ...
        data-provider-category="{{ $invoice->folder->slip->provider->provider_category->get_name()  }}"
        data-observation="{{ $invoice_line->get_observation()  }}"
        data-prestation_type="{{ $prestationable ? $prestationable->get_prestationable_type() : ''}}"
        data-service="{{ $prestationable ? $prestationable->get_fullname() : '' }}"
        ...
        >
        <td>{{ $index++ }}</td>
        <td data-bs-toggle="tooltip" data-bs-title="{{ $invoice_line->get_matching_prestation() }}" >{{ $invoice_line->get_sanitized_description() }}</td>
      ...
      {{ $gap }}
       ...
       <ul>
        ...
        @if ( $observation->title =="prestation non reconnue")
        @endif
