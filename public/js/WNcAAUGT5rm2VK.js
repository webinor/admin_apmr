$(document).ready(function() {
	...
	$("#transfert_documents").change(function(e) {
		e.preventDefault();

		// var closest_row = $(`#${$(this).val()}`).closest('.row').addClass('');

		$(".doc_type").attr("id", $(this).val());
		////console.log(closest_row);

		if ($(this).val() == "import_declaration") {
			$("#di_dates").removeClass("d-none");
		} else {
			$("#di_dates").addClass("d-none");
		}
	...
	$("#get_link").click(function(e) {
		...
		$("#get_link").toggleClass("d-none");
		$("#loader_link").toggleClass("d-none");
		...
		data_send = getInputValues('form', data_send);




		let url = `/api/transfert/get_link`,

			onSuccess = function(data, textStatus, xhr) {
				if (data.status) {


					var link = data.link; //document.getElementById("myInput");
		...
		let onSuccess = function(data, textStatus, xhr) {
				if (data.status) {
					...
					if ($('#action').val() == 'create') {
						$("#form")[0].reset();
						$('.line').remove();
						$('.transfert').val(data.data.transfert);
					} else {
