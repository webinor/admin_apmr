(function($) {
  ...
  var accounts = new Bloodhound({
   ...
   // local: states
   prefetch: { 
    url: $("#typeahead_url").val() ? $("#typeahead_url").val() :  '/api/getResources',
    cache:false,
    prepare: function (query , settings = {}) {
     // console.log(settings);
     // var related_identifier = $("#related_identifier").val();
      settings['url'] =  query.url ;// +( '&query=' + related_identifier );
     // settings['data'] =  {query:related_identifier};
      settings['headers'] = {
...
$('#form-search #qryo').typeahead({
//$('#form-search #qry').typeahead({
  hint: true,
  highlight: true,
  minLength: 1
}, {
