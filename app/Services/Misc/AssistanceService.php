    ...
    CreateVariablesResponder, ShowVariablesResponder, EditVariablesResponder, ViewsResponder
{
    ...
    public function count_filtered_results(Request $filters)
    {
        ...
        return [
            ...
            "filter-button-text" => __(
                $filtered_results . " dossier(s) trouvé(s)"
            ),
        ];
    }

    public function new_get_filtered_results(
        Request $filters,
        Assistance $binded_slip = null
    ) {
        $slip = $binded_slip
            ? $binded_slip
            : Assistance::whereCode($filters->input("slip"))->first();

        //   dd($slip);

        $query = Assistance::query()->with([
        ...
        $min_price = $filters->input("min-price")
            ? $filters->input("min-price")
            : 0;
        $max_price = $filters->input("max-price")
            ? $filters->input("max-price")
            : 200000;
        $folders = $folders->filter(function ($folder, $key) use (
            $min_price,
            $max_price
        ) {
            return $folder->get_amount() >= $min_price &&
                $folder->get_amount() <= $max_price;
        });
    ...
    function searchInvoices($slip, $query_search)
    {
        ...
        $folders = Assistance::whereHas("invoices", function ($query) use (
            $query_search
        ) {
        ...
        $folders_success = Assistance::whereHas("invoices", function (
            $query
        ) use ($query_search) {
            $query->whereReference(substr($query_search, 0, 8));
        })
        ...
        $folders_reject = Assistance::whereHas("invoices", function (
            $query
        ) use ($query_search) {
            $query->whereReference(substr($query_search, 0, 8));
        })
    ...
    function createFile($request)
    {
        try {
            ...
            $home =
                $_SERVER["SERVER_NAME"] == "192.168.43.84" ||
                /**/ $_SERVER["SERVER_NAME"] == "localhost" ||
                $_SERVER["SERVER_NAME"] == "127.0.0.1"
                    ? "marcel"
                    : ($_SERVER["SERVER_NAME"] == "51.254.121.221" ||
                    $_SERVER["SERVER_NAME"] == "51.254.121.44"
                        ? "ubuntu"
                        : "thsailid64");
            $provider = Assistance::whereName($request["provider"])
                ...
                ->first();
            $timer = null;
            $slip_identification = $request["slip"]
                ? $request["slip"]
            ...
            if (!$provider) {
                return [
                    ...
                    "errors" => [
                        "provider" => [
                            "Ce prestataire n'existe pas dans la base de données",
                        ],
                    ],
                ];
            }
            ...
            $slip = Assistance::whereIdentification(
                $slip_identification
            )->first();
                ...
                foreach ($files as $index => $file) {
                    ...
                    if (!$existing_folder) {
                        ...
                        if ($_SERVER["SERVER_ADDR"] == "127.0.0.1") {
                            //https://extract237.s3.eu-west-3.amazonaws.com/
                            ...
                            if ($request["timer"]) {
                                ...
                                $schedule->start_at = $delay->setTimezone(
                                    "Africa/Douala"
                                );
                                ...
                                Assistance::dispatch(
                                    $slip_identification,
                                    [$file["path"]],
                                    "https://extract237-pass24.s3.eu-west-3.amazonaws.com/extracted_doc/" .
                                        $file["path"],
                                    $file["name"],
                                    $home,
                                    $user,
                            ...
                            $_SERVER["SERVER_ADDR"] == "51.254.121.221" ||
                            $_SERVER["SERVER_ADDR"] == "51.254.121.44"
                        ) {
                            $s3_file = Assistance::disk("s3")->get(
                                "extracted_doc/" . $file["path"]
                            );
                            ...
                            Assistance::dispatchSync(
                                $slip_identification,
                                [$file["path"]],
                                "https://extract237.s3.eu-west-3.amazonaws.com/extracted_doc/" .
                                    $file["path"],
                                $file["name"],
                                $home,
                                $user,
                        ...
                        if (!$request["timer"]) {
                            $existing_folder->load([
                                ...
                                "user:id,code",
                                "extraction_times" => function ($query) use (
                                    $existing_folder
                                ) {
                                    $query
                                        ->select("id", "folder_id", "duration")
                            ...
                            Assistance::dispatch(
                                ...
                                $has_items,
                                $is_new_folder,
                                $user /*, $host , $remote_address/*, $_SERVER['REMOTE_ADDR'] , $_SERVER['SERVER_ADDR']*/
                            );
                        }
                    }
                }
...
if (!empty($filters["wheel_chair"])) {
    ...
    public function createCity(array $company_details)
    {
        try {
            ...
            if (
                City::select("id")
                    ->where("name", $company_details["name"])
                    ->count()
            ) {
                return [
                    "status" => false,
            ...
            foreach ($columns as $column) {
                array_key_exists($column, $company_details)
                    ? ($city->{$column} = $company_details[$column])
                    : null;
            }
